---
name: together-ai-complaints
description: 'Collect, refresh, and summarize PUBLICLY available developer feedback and complaints about Together AI (the LLM/inference API provider, together.ai). Use when building or updating the developer-complaints website, researching Together AI reliability, latency, downtime, billing, rate limits, model quality, or support issues. Gathers public posts from Reddit, Hacker News, GitHub issues/discussions, the official status page, Trustpilot/G2, and X, deduplicates them, categorizes by issue type, attributes every item to a source URL, and outputs BOTH structured JSON and Markdown. Public sources only; cite every claim.'
argument-hint: 'Optional: time window (e.g. "last 90 days") or category (e.g. "billing")'
---

# Together AI — Public Developer Complaints Collector

## When to Use
- Building or refreshing the developer-complaints website with Together AI data.
- Researching what developers publicly report about Together AI: latency, downtime, rate limits, billing, model accuracy/quality, API changes, support responsiveness.
...
1. **Reddit** — r/LocalLLaMA, r/MachineLearning, r/OpenAI, general search for "Together AI".
2. **Hacker News** — Algolia HN Search (`https://hn.algolia.com/`) for "Together AI".
3. **GitHub** — Issues/Discussions in `togethercomputer` repos and in projects that depend on Together (search "together.ai" / "together-ai" in issues).
4. **Official status page / changelog** — for confirmed incidents and downtime.
5. **Trustpilot / G2 / Capterra** — public reviews.
6. **X (Twitter)** — public posts mentioning @togethercompute / "Together AI".
...
2. Search each approved source for "Together AI" mentions that describe a problem or complaint.
...
6. Emit **both** outputs exactly as defined in [output-formats](./references/output-formats.md):
   ...
   - `together-ai-complaints.md`
7. End with a short summary: total items, breakdown by category, and date range covered.

## Output
Produce both files following [output-formats](./references/output-formats.md). The `provider` field is always `"Together AI"`.
